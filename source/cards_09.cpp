...
NumericMatrix raster_data(int iter, 
  ...
  NumericMatrix coeffs(9, layers + n_square);
  
  
  // set coefficients
  for(int i = 0; i < 9; i++) {
    for(int j = 0; j < layers + n_square; j++) {
  ...
  for(int it = 1; it < iter; it++) {
    ...
    if(variant == 0) {
      ...
      x = x + .6*x/s;
      y = y + .6*y/s;
      z = .5*z + .5*z/s + sin(square);
    } else if(variant == 1){
      x = cos(x) ;
      y = cos(y) + sin(square) - 2;
      z = 1 + sin(z) + sin(square);
    } else {
      x = 3 * cos(x);
      y = 3 * cos(y) + cos(square) - 2;
      z = 1 + sin(z) + sin(square);
    }
